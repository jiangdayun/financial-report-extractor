...
if uploaded_file is not None:
    ...
    st.write(f"命中页码：{matched_page}")
    st.text_area("命中文字", matched_text, height=400)
    st.write(f"利润表命中页码：{income_page}")
    with st.expander("查看利润表命中文字", expanded=False):
        st.write(income_text)

    st.write(f"资产负债表命中页码：{balance_page}")
    ...
    if matched_page is not None:
        ...
        if st.button("提取财务数据"):
            main_result = extract_fields(matched_text)
            income_result = extract_fields(income_text, "income")
            balance_result = extract_fields(balance_text, "balance")
